...
q.push({agent:'content-engine',message:`Content Engine: ${summary}`,ts:new Date().toISOString(),sent:false});
fs.writeFileSync(qFile,JSON.stringify(q,null,2));
"
```

Note: In practice, construct the `summary` variable from the actual output of prior steps (e.g. post topic, approval status, engagement opportunities) and embed it in the node -e command above.
